...
# --- DB Insert Queue and Worker Threads ---
...
def db_insert_worker():
    while True:
        ...
        try:
            insert_klines(symbol, interval, klines)
...
def log_weight_limit_reached(symbol, interval, current_weight):
...
def log_api_limit_error(symbol, interval, error_message):
...
def update_weight_from_headers(symbol,interval):
    ...
    try:
        if hasattr(client, '_last_response'):
            ...
            if weight:
                ...
                if weight_tracker['current_weight'] >= weight_tracker['weight_threshold']:
                    log_weight_limit_reached("GLOBAL", "ALL", weight_tracker['current_weight'])
...
#UTHO server
#connection_string_postgresql = "postgresql://lab:IndiaNepal1-@150.241.244.23:5432/labdb2"
connection_string_postgresql = "postgresql://lab:IndiaNepal1-@127.0.0.1:5432/labdb2"


def create_new_engine():
    """Create a brand-new SQLAlchemy engine for the current process.
    Use NullPool to avoid sharing pooled connections across forked processes.
    """
    return create_engine(
        ...
        # Avoid connection pooling across processes; each process gets fresh connections
        poolclass=NullPool,
        pool_pre_ping=True,
        echo=False
    )
...
def setup_header_capture(client):
...
class SQLAccessHelper:
    ...
    def fetch_dataframe(self, sql_query, params=None):
        try:
            ...
            log_db_error(e, "❌ SQL Fetch Error", 'fetch_dataframe')
    ...
    def execute(self, sql_query, params=None, autocommit=False):
        try:
            with self.connection_lock:
                self._ensure_engine()
                optimized_query = optimize_sql_query(sql_query)
                cleaned_params = clean_timestamp_values(params or {})
                with self._get_connection_with_retry() as conn:
                    if autocommit:
            ...
            log_db_error(e, "❌ SQL Execute Error", 'execute')
    ...
    def execute_many(self, sql_query, param_list, autocommit=False):
        try:
            with self.connection_lock:
                self._ensure_engine()
                optimized_query = optimize_sql_query(sql_query)
                cleaned_param_list = [clean_timestamp_values(params) for params in param_list]
                with self._get_connection_with_retry() as conn:
                    if autocommit:
                        conn = conn.execution_options(isolation_level="AUTOCOMMIT")
            ...
            log_db_error(e, "❌ SQL Executemany Error", 'execute_many')
    ...
    def fetch_one(self, sql_query, params=None):
        try:
            ...
            log_db_error(e, "❌ SQL Fetch One Error", 'fetch_one')
    ...
    def fetch_all(self, sql_query, params=None):
        try:
            ...
            log_db_error(e, "❌ SQL Fetch All Error", 'fetch_all')
    ...
    def fetch_all_safe(self, sql_query, params=None, timeout=5, retries=3, delay=2, tag=""):
        for attempt in range(1, retries + 1):
            try:
                ...
                log_db_error(e, f"❌ SQL FetchAll Error Try {attempt}", tag)
    ...
    def fetch_one_safe(self, sql_query, params=None, timeout=5, retries=3, delay=2, tag=""):
        for attempt in range(1, retries + 1):
            try:
                ...
                log_db_error(e, f"❌ SQL FetchOne Error Try {attempt}", tag)
        ...
        for attempt in range(1, retries + 1):
            try:
                with self.connection_lock:
                    ...
                    cleaned_params = clean_timestamp_values(params) if params else {}
                    with self._get_connection_with_retry() as conn:
                        if autocommit:
                            conn = conn.execution_options(isolation_level="AUTOCOMMIT")
                ...
                log_db_error(e, f"❌ SQL Execute Error Try {attempt}", tag)
    ...
    def execute_many_safe(self, sql_query, param_list, autocommit=False, timeout=5, retries=3, delay=2, tag=""):
        for attempt in range(1, retries + 1):
            try:
                ...
                log_db_error(e, f"❌ SQL ExecuteMany Error Try {attempt}", tag)
...
# --- Logger ---
def log_db_error(error, context, pair):
...
def write_db_insert_debug_log(message):
...
def table_exists(symbol, interval):
    try:
        ...
        log_db_error(e, "❌  table_exists Error for", symbol)            
        print(f"❌ table_exists Error for {symbol}-{interval}: {e}")
        return False

def is_data_up_to_date(symbol, interval):
    try:
        ...
        return latest_time_ms >= expected_time - interval_ms

    except Exception as e:
        log_db_error(e, "❌  is_data_up_to_date Error for", symbol)            
        print(f"❌ is_data_up_to_date Error for {symbol}-{interval}: {e}")
        return False

def fetch_data_from_db(symbol, interval, limit):
    try:
        ...
        log_db_error(e, "❌  fetch_data_from_db Error for", symbol)   
        print(f"❌ fetch_data_from_db Error for {symbol}-{interval}: {e}")
        return pd.DataFrame()

# --- INSERT KLINES (PostgreSQL compatible) ---
def insert_klines(symbol, interval, klines):
    try:
        ...
        log_db_error(e, "❌ Insert Klines Error for", symbol)
        print(f"❌ Insert Klines Error for {symbol}-{interval}: {e}")

def insert_or_update_pair_status(pair_status_data):
    try:
        ...
        log_db_error(e, "❌ Insert PairStatus Error", "n/a")
        print(f"❌ Insert PairStatus Error: {e}")

def get_existing_pair_symbols():
    try:
        sql = "SELECT pair, active_squeeze_trend FROM pairstatus"
        ...
        log_db_error(e, "❌ Select get_existing_pair_symbols Error", "n/a")
        return set()

def tradeexistdb(symbol, tf):
    """
    ...
    try:
        ...
        log_db_error(e, "❌ get_running_symbols Error", 'vv')
        print(f"❌ get_running_symbols Error: {e}")
        return False

def fetch_active_pairs_from_db():
    try:
        sql = """
        ...
        log_db_error(e, "fetch_active_pairs_from_db", "DB Query")
        return []

def fetch_price_precision_from_db(pair):
    try:
        sql = """
        ...
        if result:
            ...
            log_db_error(Exception(f"No price_precision found for {pair}"), 'fetch_price_precision_from_db', f"Fallback to default precision")
            return 4
    except Exception as e:
        log_db_error(e, "fetch_price_precision_from_db", pair)
        return 4

def fetch_qty_precision_from_db(pair):
    try:
        sql = """
        ...
        if result:
            ...
            log_db_error(Exception(f"No quantity_precision found for {pair}"), 'fetch_qty_precision_from_db', f"Fallback to default")
            return 0
    except Exception as e:
        log_db_error(e, "fetch_qty_precision_from_db", pair)
        return 0

def fetch_single_pair_from_db(pair):
    """
...
def fetch_data_safe(symbol, interval, limit):
    ...
    try:
        ...
        update_weight_from_headers(symbol,interval)
        ...
        # ✅ Queue the insert instead of starting a new thread
        insert_queue.put((symbol, interval, closed_klines))
        ...
        if "429" in error_str or "Too many requests" in error_str:
            print(f"🚨 429 error in fetch_data_safe for {symbol}-{interval}: {e}")
            log_api_limit_error(symbol, interval, error_str)
            return None
        else:
            # Other errors
            log_db_error(e, "❌ fetch_data_safe Error for", symbol)
            print(f"❌ fetch_data_safe Error for {symbol}-{interval}: {e}")
            switch_api_key()
            return None

def fetch_data_safe_for_machines(symbol, interval, limit):
    try:
        # print(f"✅ Fetching from database for {symbol}-{interval}")
        return fetch_data_from_db(symbol, interval, limit)
    except Exception as e:
        log_db_error(e, "❌ fetch_data_safe Error for", symbol)   
        print(f"❌ fetch_data_safe Error for {symbol}-{interval}: {e}")
...
def fetch_data_from_machine(machine_id, app_start):
    try:
        # query = f"SELECT * FROM {machine_id.lower()} WHERE type = 'assign'"
        # query = f"SELECT * FROM {machine_id} WHERE  type = 'running' and hedge_1_1_bool = '1' "
        ...
        log_db_error(e, "❌ fetch_data_from_machine Error for", machine_id)   
        print(f"❌ fetch_data_from_machine Error for {machine_id}: {e}")
        return []

def fetch_hedge_data_from_machine(machine_id, app_start):
    try:
        print(f"🔍 fetch_hedge_data_from_machine called for {machine_id}, app_start={app_start}")
        
        # First check if table exists
        table_check_query = f"SELECT COUNT(*) FROM information_schema.tables WHERE table_name = '{machine_id.lower()}'"
        table_exists_result = sql_helper.fetch_one_safe(table_check_query, tag=f"{machine_id}_table_check")
        ...
        for i, query in enumerate(queries_to_try):
            try:
                ...
                if rows is not None and len(rows) > 0:
                    ...
                    print(f"⚠️ Query variation {i+1} returned no rows")
        ...
        log_db_error(e, "❌ fetch_hedge_data_from_machine Error for", machine_id)   
        print(f"❌ fetch_hedge_data_from_machine Error for {machine_id}: {e}")
        print(f"📋 Full traceback: {traceback.format_exc()}")
        return []

# --- Assign Trade To Machine ---
def _assign_trade_to_machine(df, symbol, interval, stopPrice, action, signalFrom, closes3, candle_type=None):
    try:
        if closes3 is None:
            closes3 = []
        ...
        for row in rows:
            ...
            try:
                ...
                log_db_error(e, f"Error parsing timestamp for {candel_time}", symbol)
            ...
            try:
                exists_query = """
                    ...
                    WHERE pair = :pair AND action = :action AND signalfrom = :signalFrom
                          AND interval = :interval AND type NOT IN ('close', 'hedge_close') AND hedge = 0
                """
                ...
                log_db_error(e, f"Error checking for existing records: {e}", symbol)
            ...
            document = {
                ...
                "profit_journey": 0, "min_profit": 0, "hedge_order_size": 0, "hedge_1_1_bool": 0,
                "added_qty": 0, "min_comm_after_hedge": 0, "type": "assign", "min_close": "min_close",
            ...
            cleaned_document = clean_timestamp_values(document)
            ...
            for attempt in range(1, 101):
                try:
                    ...
                    if result1 and result2 and result3:
                        print(f" Attempt {attempt}: Data inserted for {unique_id}")
                        success = True
                        break
                    else:
                    ...
                    log_db_error(e, f" Attempt {attempt}: Insert failed for {unique_id}. Error: {e}", symbol)
                    time.sleep(5)
                    last_error = str(e)
            ...
            if success:
                ...
                log_db_error(Exception(f"Failed to insert after 100 attempts for {unique_id}"), f"Failed to insert after 100 attempts for {unique_id}", symbol)
        ...
        log_db_error(e, f"Unhandled error in _assign_trade_to_machine: {e}", symbol)
        return None, f"Unhandled error: {e}"

def AssignTradeToMachineLAB(df, symbol, interval, stopPrice, action, signalFrom, closes3, candle_type):
    return _assign_trade_to_machine(df, symbol, interval, stopPrice, action, signalFrom, closes3, candle_type)

# --- Update Machine Table From All Pairs ---
def update_table_from_all_pairs(all_pairs, machine_id):
    try:
        if not all_pairs:
            return
        ...
        for uid, item in all_pairs.items():
            if item and item.get('unique_id'):
                ...
                converted_item = convert_boolean_to_int(item)
            ...
            try:
                ...
                for uid, item in batch:
                    try:
                        ...
                        batch_errors += 1
                        log_db_error(e, "update_table_from_all_pairs", f"UID: {uid}")
                ...
                if batch_time > 60:  # 10 second timeout per batch
                    log_db_error(Exception(f"Batch timeout after {batch_time:.2f}s"), "update_table_from_all_pairs", "batch_timeout")
                    break
                    
            except Exception as batch_e:
                log_db_error(batch_e, "update_table_from_all_pairs", f"Batch {i//batch_size + 1}")
        ...
        #     except Exception as e:
        #         log_db_error(e, "update_table_from_all_pairs", "batch_timeout")

    except Exception as outer_e:
        log_db_error(outer_e, "update_table_from_all_pairs", machine_id)
        print(f"❌ update_table_from_all_pairs Global Error for {machine_id}: {outer_e}")

def convert_boolean_to_int(data_dict):
    """
...
def olab_update_single_uid_in_table(uid, all_pairs, machine_id):
    try:
        ...
        log_db_error(e, "update_single_uid_in_table", f"UID: {uid}")
        print(f"❌ Exception during update for UID {uid}: {str(e)}")

        
def update_tmux_log(code):
    try:
        ...
        log_db_error(e, "update_tmux_log", "SQL execution issue")
        print(f"❌ Error updating tmux_log: {e}")

# --- Update All Trade Table ---
def update_all_trade_table():
    try:
        ...
        log_db_error(e, "update_all_trade_table", "SQL execution issue")

def insert_bot_error_log(uid, source, message, timestamp, machine_id, json_error=None):
    try:
        query = """
        ...
        log_db_error(e, "insert_bot_error_log", "SQL execution issue")

def insert_bot_event_log(uid, source, Pl_after_comm, plain_message, json_message, timestamp, machine_id):
    try:
        ...
        log_db_error(e, "insert_bot_event_log", "insert_bot_event_log")
        print(f"❌ Failed to insert bot_event_log: {e}")

def insert_system_metrics(cpu_usage, memory_usage, thread_count, machine_id):
    try:
        query = """
        ...
        log_db_error(e, "insert_system_metrics", "insert_system_metrics")
        print(f"❌ Failed to insert system metrics: {e}")

def update_squeeze_status(pair, squeeze_status, squeeze_value, active_squeeze, active_squeeze_trend):
    """
    ...
    try:
        ...
        log_db_error(e, "❌ Update squeeze status error", pair)
        print(f"❌ Error updating squeeze status for {pair}: {e}")



def fetch_squeezed_pairs_from_db_paginated(offset=0, limit=10):
    """
    ...
    try:
        sql = """
            SELECT pair, status1d_4h, status4h_1h, Last_day_close_price, 
                   tf_1d_trend, squeeze_value, active_squeeze, active_squeeze_trend, 
                   overall_trend_RC, overall_trend_percentage_RC, 
                   overall_trend_HC, overall_trend_percentage_HC,
                   overall_trend_4h, overall_trend_percentage_4h,
                   overall_trend_1h, overall_trend_percentage_1h,
                   volume_1h
            FROM pairstatus 
            WHERE squeeze = TRUE
        ...
        """
        results = sql_helper.fetch_all(sql, {"offset": offset, "limit": limit})
        return [{
            'pair': row[0],
            'status1d_4h': row[1],
            'status4h_1h': row[2],
            'Last_day_close_price': row[3],
            'tf_1d_trend': row[4],
            'squeeze_value': row[5],
            'active_squeeze': row[6],
            'active_squeeze_trend': row[7],
            'overall_trend_RC': row[8],
            'overall_trend_percentage_RC': row[9],
            'overall_trend_HC': row[10],
            'overall_trend_percentage_HC': row[11],
            'overall_trend_4h': row[12],
            'overall_trend_percentage_4h': row[13],
            'overall_trend_1h': row[14],
            'overall_trend_percentage_1h': row[15],
            'volume_1h': row[16]
        } for row in results]
    except Exception as e:
        log_db_error(e, "fetch_squeezed_pairs_from_db_paginated", "DB Query")
        return []


def fetch_non_squeezed_pairs_from_db():
    try:
        ...
        log_db_error(e, "fetch_non_squeezed_pairs_from_db", "DB Query")
        return []


def fetch_squeezed_pairs_from_db():
    try:
        ...
        log_db_error(e, "fetch_squeezed_pairs_from_db", "DB Query")
        return []
...
# def fetch_non_squeezed_pairs_from_db():
#     """
#     Fetch all pairs that are NOT in squeeze state.
...
#     try:
#         sql = """
#             SELECT pair, status1d_4h, status4h_1h, Last_day_close_price, tf_1d_trend, squeeze_value, active_squeeze,
#             active_squeeze_trend, overall_trend_RC, overall_trend_percentage_RC, overall_trend_HC,	
#             overall_trend_percentage_HC, overall_trend_4h, overall_trend_percentage_4h, overall_trend_1h, overall_trend_percentage_1h
#             FROM pairstatus 
#             WHERE squeeze = FALSE OR squeeze IS NULL
#         """
#         results = sql_helper.fetch_all(sql)
#         return [{'pair': row[0], 'status1d_4h': row[1], 'status4h_1h': row[2], 'Last_day_close_price': row[3], 
#         'tf_1d_trend': row[4], 'squeeze_value': row[5], 'active_squeeze': row[6], 'active_squeeze_trend': row[7],
#         'overall_trend_RC': row[8], 'overall_trend_percentage_RC': row[9], 'overall_trend_HC': row[10], 'overall_trend_percentage_HC': row[11],
#         'overall_trend_4h': row[12], 'overall_trend_percentage_4h': row[13], 'overall_trend_1h': row[14], 'overall_trend_percentage_1h': row[15]} 
#         for row in results]
#     except Exception as e:
#         log_db_error(e, "fetch_non_squeezed_pairs_from_db", "DB Query")
#         return []
...
# def fetch_squeezed_pairs_from_db():
#     """
#     Fetch all pairs that are currently in squeeze state.
...
#         sql = """
#             SELECT pair, status1d_4h, status4h_1h, Last_day_close_price, 
#             tf_1d_trend, squeeze_value, active_squeeze, active_squeeze_trend, overall_trend_RC,	
#             overall_trend_percentage_RC, overall_trend_HC, overall_trend_percentage_HC,
#             overall_trend_4h, overall_trend_percentage_4h, overall_trend_1h, overall_trend_percentage_1h
#             FROM pairstatus 
#             WHERE squeeze = TRUE
#         """
#         results = sql_helper.fetch_all(sql)
#         return [{'pair': row[0], 'status1d_4h': row[1], 'status4h_1h': row[2], 'Last_day_close_price': row[3], 
#         'tf_1d_trend': row[4], 'squeeze_value': row[5], 'active_squeeze': row[6], 'active_squeeze_trend': row[7],
#         'overall_trend_RC': row[8], 'overall_trend_percentage_RC': row[9], 'overall_trend_HC': row[10],
#         'overall_trend_percentage_HC': row[11], 'overall_trend_4h': row[12], 'overall_trend_percentage_4h': row[13],
#         'overall_trend_1h': row[14], 'overall_trend_percentage_1h': row[15]} for row in results]
#     except Exception as e:
#         log_db_error(e, "fetch_squeezed_pairs_from_db", "DB Query")
#         return []

        
def fetch_non_squeezed_pairs_from_db_paginated(offset=0, limit=10):
    """
    ...
    try:
        sql = """
            SELECT pair, status1d_4h, status4h_1h, Last_day_close_price, tf_1d_trend, squeeze_value,
                   active_squeeze, active_squeeze_trend, overall_trend_RC, overall_trend_percentage_RC,
                   overall_trend_HC, overall_trend_percentage_HC, overall_trend_4h,
                   overall_trend_percentage_4h, overall_trend_1h, overall_trend_percentage_1h,
                   volume_1h
            FROM pairstatus 
            WHERE squeeze = FALSE OR squeeze IS NULL
        ...
        log_db_error(e, "fetch_non_squeezed_pairs_from_db_paginated", "DB Query")
        return []

def get_total_pairs_count():
    """
    ...
    try:
        ...
        log_db_error(e, "get_total_pairs_count", "DB Query")
        return 0

def check_running_trade_exists(symbol, interval, signal_from, candle_type):
    """
    ...
    try:
        sql = """
            ...
            AND unique_id LIKE :candle_type
        """
        ...
        result = sql_helper.fetch_one(sql, {
            ...
            'candle_type': f'%{candle_type}%'  # ensures substring match
        })
        ...
        log_db_error(e, "check_running_trade_exists Error", symbol)
        print(f"❌ check_running_trade_exists Error for {symbol}-{interval}: {e}")
        return False

def buy_is_loss_exceeding_5_percent_with_min_trades():
    """
    Returns True if:
    ...
    """
    query = """
        SELECT pl_after_comm FROM alltraderecords
    ...
    try:
        ...
        log_db_error(e, "❌  fetch_data_from_db Error for", 'check20mintrades')  
        print(f"[SQL ERROR] {e}")
        return False
    ...
    resultpl = float(total_loss) < (0.05 * float(total_profit))
    print((f'is_loss_exceeding_5_percent_with_min_trades - {total_profit} - {total_loss} - total_loss < (0.05 * total_profit) = {resultpl}'))
    
    write_db_insert_debug_log(f'is_loss_exceeding_5_percent_with_min_trades - {total_profit} - {total_loss} - total_loss < (0.05 * total_profit) = {resultpl}')

    return resultpl

def sell_is_loss_exceeding_5_percent_with_min_trades():
    """
    Returns True if:
    - There are at least 20 running, non-hedged trades, and
    - Total loss exceeds 5% of total profit
...
def update_database_to_release_hedge(unique_id, commission, signal, pl_after_comm, new_stop_price, Hed_pl, profitLoss, machine_id):
    try:
        if signal == 'BUY':
            update_query = f"""
                UPDATE {machine_id.lower()} SET 
        ...
        log_db_error(outer_e, "update_database_to_release_hedge", machine_id)
        print(f"❌ update_database_to_release_hedge Global Error for {machine_id}: {outer_e}")

def bulk_fetch_kline_data(symbols, interval, limit):
    """
    ...
    try:
        ...
        log_db_error(e, "❌ bulk_fetch_kline_data Error", f"Symbols: {','.join(symbols)}")
        print(f"❌ bulk_fetch_kline_data Error for symbols: {e}")
        return pd.DataFrame()

def bulk_update_squeeze_status(updates):
    """
    ...
    try:
        ...
        if trans is not None:
            try:
                ...
                pass
        log_db_error(e, "❌ bulk_update_squeeze_status Error", "n/a")
        raise
    finally:
        if conn is not None:
            try:
...
def update_overall_4h_market_trends_db(overall_4h_trend, overall_4h_percentage):
    """
    ...
    try:
        ...
        log_db_error(e, "update_overall_4h_market_trends_db", "all_pairs")
        print(f"❌ Error updating overall 4H market trends: {e}")

def update_overall_1h_market_trends_db(overall_1h_trend, overall_1h_percentage):
    """
    ...
    try:
        ...
        log_db_error(e, "update_overall_1h_market_trends_db", "all_pairs")
        print(f"❌ Error updating overall 1H market trends: {e}")

def check_signal_processing_log_exists(symbol, interval, candle_pattern, candle_time):
    ...
    try:
        ...
        WHERE symbol = :symbol 
        AND interval = :interval 
        AND candle_pattern = :candle_pattern
...
def insert_signal_processing_log(candle_time, symbol, interval, signal_type, signal_source, 
...
def insert_signal_validation_log(symbol, interval, validation_step, validation_result, 
...
def insert_performance_metric(metric_type, metric_name, metric_value, metric_unit, 
...
def insert_batch_processing_log(batch_type, batch_size, successful_count, error_count, 
...
def insert_system_health_log(cpu_usage, memory_usage, memory_available_mb, disk_usage, 
...
def insert_cache_performance_log(cache_type, operation_type, symbol, interval, cache_key, 
...
def log_error(error, context, symbol=None, machine_id=None):
    """Log error with context"""
    try:
        # Use the existing log_db_error function
        log_db_error(error, context, symbol)
        return True
    except Exception as e:
        print(f"❌ Failed to log error: {e}")
        return False

def log_signal_processing(candle_time, symbol, interval, signal_type, signal_source, 
                         candle_pattern, price, squeeze_status, active_squeeze, 
                         processing_time_ms, machine_id, timestamp, json_data=None, unique_id=None):
    """Log signal processing"""
    try:
        return insert_signal_processing_log(candle_time, symbol, interval, signal_type, signal_source, 
                                         candle_pattern, price, squeeze_status, active_squeeze, 
                                         processing_time_ms, machine_id, timestamp, json_data, unique_id)
    except Exception as e:
...
def log_signal_validation(symbol, interval, validation_step, validation_result, 
    ...
    try:
        return insert_signal_validation_log(symbol, interval, validation_step, validation_result, 
                                         validation_value, validation_threshold, validation_message, 
                                         machine_id, timestamp, json_data)
    except Exception as e:
...
def log_performance_metric(metric_type, metric_name, metric_value, metric_unit, 
                          symbol, interval, batch_size, machine_id, timestamp, 
                          additional_data=None):
    """Log performance metric"""
    try:
        return insert_performance_metric(metric_type, metric_name, metric_value, metric_unit, 
                                      symbol, interval, batch_size, machine_id, timestamp, 
                                      additional_data)
    except Exception as e:
...
def log_batch_processing(batch_type, batch_size, successful_count, error_count, 
    ...
    try:
        return insert_batch_processing_log(batch_type, batch_size, successful_count, error_count, 
                                        crash_count, total_processing_time_ms, average_time_per_item_ms, 
                                        machine_id, timestamp, executor_type, worker_count, json_details)
    except Exception as e:
...
def log_system_health(cpu_usage, memory_usage, memory_available_mb, disk_usage, 
                     network_latency_ms, active_threads, active_processes, 
                     database_connections, api_calls_per_minute, error_rate_percent, 
                     machine_id, timestamp, health_status):
    """Log system health"""
    try:
        return insert_system_health_log(cpu_usage, memory_usage, memory_available_mb, disk_usage, 
                                     network_latency_ms, active_threads, active_processes, 
                                     database_connections, api_calls_per_minute, error_rate_percent, 
                                     machine_id, timestamp, health_status)
    except Exception as e:
...
def log_cache_performance(cache_type, operation_type, symbol, interval, cache_key, 
                         response_time_ms, cache_size_mb, hit_rate_percent, 
                         machine_id, timestamp):
    """Log cache performance"""
    try:
        return insert_cache_performance_log(cache_type, operation_type, symbol, interval, cache_key, 
                                         response_time_ms, cache_size_mb, hit_rate_percent, 
                                         machine_id, timestamp)
    except Exception as e:
...
def performance_monitor(metric_type, function_name, machine_id=None):
...
def insert_enhanced_error_log(error_level, error_category, symbol, source_function, 
    ...
    try:
        ...
        log_db_error(e, "insert_enhanced_error_log", symbol)

def update_overall_trends_for_all_pairs_db(overall_trend_RC, overall_trend_percentage_RC, overall_trend_HC, overall_trend_percentage_HC):
    """
...
def fetch_hedge_1_1():
    ...
    try:
        ...
        log_db_error(outer_e, "fetch_hedge_1_1", machine_id)
        print(f"❌ fetch_hedge_1_1 Global Error for {machine_id}: {outer_e}")

def fetch_hedge_data_for_release(pair):
    try:
        ...
        log_db_error(outer_e, "fetch_hedge_data_for_release", "system")
        print(f"❌ fetch_hedge_data_for_release Global Error: {outer_e}")
        return []

# Add timestamp cleaning function
def clean_timestamp_values(data):
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            if isinstance(value, str) and value.upper() == 'NONE':
                ...
                cleaned[key] = clean_timestamp_values(value)
            else:
                cleaned[key] = value
        return cleaned
    elif isinstance(data, list):
        return [clean_timestamp_values(item) for item in data]
    else:
        return data
