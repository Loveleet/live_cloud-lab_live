...
def table_record_exists(symbol, interval):
    """Return exact COUNT(*) for public.kline_{symbol}_{interval}. If table doesn't exist, return 0."""
    try:
        tbl = f"kline_{symbol.lower()}_{interval.lower()}"
        if not re.fullmatch(r"[a-z0-9_]+", tbl):
            raise ValueError(f"Unsafe table name: {tbl}")

        # 1) Does table exist? (inline string, no params)
        exists_rows = sql_helper.fetch_all(
            f"SELECT to_regclass('public.{tbl}')"
        )
        if not exists_rows or exists_rows[0][0] is None:
            return 0

        # 2) Exact count
        count_rows = sql_helper.fetch_all(
            f'SELECT COUNT(*)::bigint FROM public."{tbl}";'
        )
        return int(count_rows[0][0])

    except Exception as e:
        log_db_error(e, "❌ table_row_count Error for", symbol)
        print(f"❌ table_row_count Error for {symbol}-{interval}: {e}")
        return 0
...
def fetch_history_db(symbol, interval, limit):
    try:
        # PostgreSQL syntax: LIMIT instead of TOP
        sql = f"SELECT * FROM kline_{symbol.lower()}_{interval} ORDER BY time DESC"
        ...
        log_db_error(e, "❌  fetch_history_db Error for", symbol)   
        print(f"❌ fetch_history_db Error for {symbol}-{interval}: {e}")
        return pd.DataFrame()

def fetch_history_db_range(symbol, interval, start_time, end_time):
    try:
        ...
        """
        params = {
            "start_time": pd.Timestamp(start_time).tz_convert(None) if isinstance(start_time, pd.Timestamp) and start_time.tzinfo else pd.Timestamp(start_time),
            "end_time": pd.Timestamp(end_time).tz_convert(None) if isinstance(end_time, pd.Timestamp) and end_time.tzinfo else pd.Timestamp(end_time)
        }
        df = sql_helper.fetch_dataframe(sql, params=params)
        ...
        log_db_error(e, "❌  fetch_history_db_range Error for", symbol)   
        print(f"❌ fetch_history_db_range Error for {symbol}-{interval}: {e}")
        return pd.DataFrame()

def fetch_time_bounds(symbol, interval):
    try:
        table_name = f"kline_{symbol.lower()}_{interval}"
        sql = f"SELECT MIN(time) AS min_time, MAX(time) AS max_time FROM {table_name}"
        df = sql_helper.fetch_dataframe(sql)
        if df.empty or pd.isna(df.iloc[0]['min_time']) or pd.isna(df.iloc[0]['max_time']):
            return None, None
        min_time = pd.to_datetime(df.iloc[0]['min_time'], utc=True)
        max_time = pd.to_datetime(df.iloc[0]['max_time'], utc=True)
        return min_time, max_time
    except Exception as e:
        log_db_error(e, "❌  fetch_time_bounds Error for", symbol)   
        print(f"❌ fetch_time_bounds Error for {symbol}-{interval}: {e}")
        return None, None
...
def fetch_data_safe(symbol, interval, limit):
    ...
    try:
        ...
        if is_data_up_to_date(symbol, interval):
            ...
            df = fetch_data_from_db(symbol, interval, limit)
        ...
        try:
            insert_klines(symbol, interval, closed_klines)
        except Exception as e:
...
def fetch_data_safe_for_machines(symbol, interval, limit):
    try:
        ...
        error_str = str(e)
        if "429" in error_str or "Too many requests" in error_str:
            print(f"🚨 429 error in fetch_data_safe_for_machines for {symbol}-{interval}: {e}")
            log_api_limit_error(symbol, interval, error_str)
            return None
        else:
            log_db_error(e, "❌ fetch_data_safe_for_machines Error for", symbol)
            print(f"❌ fetch_data_safe_for_machines Error for {symbol}-{interval}: {e}")
            switch_api_key()
            return None
        ...
        for row in rows:
            ...
            for attempt in range(1, 2):
                try:
                    ...
                    # log_db_error(e, f"Duplicate detected for {unique_id}, skipping insert", symbol)
                    return unique_id, "Already exists"                    
                        
                except Exception as e:
                    log_db_error(e, f" Attempt : Insert failed for {unique_id}. Error: {e}", symbol)
                    time.sleep(1)
                    last_error = str(e)
...
def count_running_trades(action):
    """
    ...
    try:
        ...
        log_db_error(e, "count_running_trades", "DB Query")
        return 0
    

def count_running_trades_negative(action):
    """
    Get the total count of running trades for a given action.
...
def buy_is_loss_exceeding_5_percent_with_min_trades():
    ...
    resultpl = float(total_loss) < (0.06 * float(total_profit))
    ...
    if resultpl and CandleColor('BTCUSDT','1h') == 'GREEN':
        updateSuperTrend('BUY')

    return resultpl

def getSuperTrend(minutes=15):
    try:
        ...
        log_db_error(e, "getSuperTrend Error", "DB Query")
        return None

def getSuperTrendPercent(percent=30):
    try:
        ...
        log_db_error(e, "getSuperTrendPercent Error", "DB Query")
        print(f"[SQL ERROR] {e}")
        return False
    

def check_and_deactivate_supertrend(percent=5):
    try:
...
def sell_is_loss_exceeding_5_percent_with_min_trades():
    ...
    write_db_insert_debug_log(f'is_loss_exceeding_5_percent_with_min_trades - {total_profit} - {total_loss} - total_loss < (0.05 * total_profit) = {resultpl}')
    if resultpl and CandleColor('BTCUSDT','1h') == 'RED':    
        updateSuperTrend('SELL')
    return resultpl

def updateSuperTrend(action):
    try:
        timestamp = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')
        update_query = """
            UPDATE supertrend 
            ...
            WHERE active = FALSE
        """
        sql_helper.execute(update_query, {'now': timestamp, 'action': action},autocommit=True)
        with open('log_event/superTrend.txt', 'a', encoding="utf-8") as f:
                f.write(f"Trend Active Time: {timestamp} -- Trend : {action} --'\n")

    except Exception as e:
        log_db_error(e, "❌ updateSuperTrend Error for", 'updateSuperTrend')  
        print(f"[SQL ERROR] {e}")
        return False

def deactivebothposition():
    try:
        update_query = """
            UPDATE supertrend 
        ...
        log_db_error(e, "❌ deactivebothposition Error for", 'deactivebothposition')  
        print(f"[SQL ERROR] {e}")
        return False
    
def activebothposition():
    try:
        update_query = """
            UPDATE supertrend 
...
def CandleColor(symbol,min):
    df=fetch_data_safe(symbol,min,10)

    # df = convertToHenkin(df)
